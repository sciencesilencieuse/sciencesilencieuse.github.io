...
## Détour par la physique statitique
...
| | Physique statistique (réseau) | Théorie des champs (continuum) |
|---|---|---|
| Source | $J_i$ | $J(x)$ |
| Génératrice | $Z(J)$ | $Z[J]$ |
| Fonctions de Green | $G_{i_1\cdots i_n} = \langle\hat\phi_{i_1}\cdots\hat\phi_{i_n}\rangle_t$ | $G^{(n)}(x_1,\ldots,x_n) = \langle\Omega\|T\hat\phi(x_1)\cdots\hat\phi(x_n)\|\Omega\rangle$ |
| Recette | $\dfrac{1}{Z(0)}\dfrac{\partial^n Z}{\partial J_{i_1}\cdots\partial J_{i_n}}\bigg{\|}\_{0}$  | $\dfrac{1}{\mathrm{i}^n}\dfrac{1}{Z[0]}\dfrac{\delta^n Z[J]}{\delta J(x_1)\cdots\delta J(x_n)}\bigg{\|}_{0}$ |
| Ordre | $\langle\hat\phi_i\rangle_t \neq 0$ | $\langle\Omega\|\hat\phi(x)\|\Omega\rangle \neq 0$ |
...
On représente d'interaction, au premier ordre (la série de Dyson, tronquée)&nbsp;:
...
<p style="text-align:center;">
$\displaystyle
\delta\langle\hat x(t)\rangle = i\int\mathrm{d}t'\,\theta(t-t')\,\big\langle\big[\hat x_I(t), \hat x_I(t')\big]\big\rangle\, f(t')\\z
\Longrightarrow
...
Conséquence remarquable, objet des questions&nbsp;: ce nombre ne dépend pas de l'état . <b>La réponse est la même à toute température</b>.
...
Et pour $t>0$, on récupère un pôle imaginaire pur $\Rightarrow$ aucune oscillation, une <b>relaxation pure</b> $e^{-D\mathbf q^2 t}$. Et cela donne une gaussienne qui s'étale dans l'espace réel $G \propto t^{-3/2}\,e^{-|\mathbf x|^2/4Dt}$.</li>
</ul>
...
[^g1]: Un mode hydrodynamique est la fluctuation d'une quantité conservée (le nombre de particules, l'énergie, l'impulsion).
